...
1. **File menu:** right-click the PDF → **Convert to Markdown (Distill)**.
   → expect `<name>.pdf.md` to appear, opened in a new pane, with YAML frontmatter.
2. **Folder menu:** right-click a folder → **Distill: convert all attachments**
   → converts every convertible file recursively; a notice reports the count.
3. **Command palette:** open a convertible file, then ⌘P → **Distill: Convert file**.

## 3. Clipboard conversion
1. Copy some HTML content from a web page (or plain text).
2. ⌘P → **Distill: Convert clipboard**.
3. Expect a new note named `Clipboard <timestamp>.md` to appear with the converted content.
4. The temporary source file (`_distill_clip_*.html`) should be cleaned up automatically.

## 4. Refinery
### Enable
1. Settings → **Distill** → toggle **Enable Refinery** on.
...
## 5. Settings to verify (Settings → Distill)
